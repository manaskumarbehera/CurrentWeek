...
// These are the real shared implementations (week.js), exercised directly so
// the tests guard the exact code the extension runs.
const {
  getCurrentWeekNumber,
  getISOWeeksInYear,
  getISOMonday,
  getDateOfISOWeek: weekNumberToMonday,
} = require("../week");

// ---------------------------------------------------------------------------
// Helpers (DOM-free, for exploratory checks)
// ---------------------------------------------------------------------------
...
describe("Scenario 4: Walking every day of 2023 (52-week year) produces exactly weeks 1–52", () => {
  ...
  test.each(Array.from({ length: 52 }, (_, i) => [i + 1]))("week %i is present", (w) => {
    expect(ownWeeks.has(w)).toBe(true);
  });
});
...
describe('Scenario 5: "Today" always produces a consistent week for known dates', () => {
  ...
  test.each(knownDates)("%i-%i-%i → week %i", (y, m, d, expected) => {
...
describe("Exploratory: Jan 4 is always in week 1 (ISO 8601 anchor)", () => {
  ...
  test.each(years.map((y) => [y]))("Jan 4 %i → week 1", (year) => {
...
describe("Exploratory: Dec 28 of year Y is always in the last ISO week of Y", () => {
  ...
  test.each(years.map((y) => [y]))("Dec 28 %i → getISOWeeksInYear(%i)", (year) => {
...
describe("Exploratory: weekNumberToMonday ↔ getCurrentWeekNumber round-trip", () => {
  ...
  test.each(years.map((y) => [y]))("every week in %i survives a round-trip", (year) => {
