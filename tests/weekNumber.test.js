const { getCurrentWeekNumber, getISOWeeksInYear, getISOMonday } = require("../week");
...
describe("getISOMonday — (getDay()+6)%7 formula", () => {
  ...
  test("Sunday is in the same ISO week as the 6 days before it", () => {
    ...
    const monday = new Date("2026-03-09");
    expect(getCurrentWeekNumber(getISOMonday(sunday))).toBe(
      getCurrentWeekNumber(getISOMonday(monday))
    );
  });
});
